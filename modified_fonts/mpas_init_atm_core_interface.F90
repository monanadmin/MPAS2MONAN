...
module init_atm_core_interface
   ...
   !The old includes now are modules:
   use modCoreVariables
   use modDomainVariables
   use modSetupImmutableStreams
   use modBlockDimensionRoutines
   use modDefinePackages
   use modStructsAndVariables
   use modNamelistCall
   use modNamelistDefines
...
#:if defined('MPAS_DEBUG')
                          'debug')
#:else
                          'optimize')
#:endif
      call mpas_log_write('  I/O layer: ' // &
#:if defined('MPAS_PIO_SUPPORT')
#:if defined('USE_PIO2')
                          'PIO 2.x')
#:else
                          'PIO 1.x')
